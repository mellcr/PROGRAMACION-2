...
void quick_sort(int*dates, char**tweets, char***hashtags, int izq, int der){
    int pivote, limit;  //no son lo mismo , el lim es un limite -> pivote : (izq+der)/2
    if(izq >= der) return;  //condicion de parada 
    
    pivote = (izq+der) /2;
    //intercambia el pivote con la primera posicion 
    swap2(dates[izq],dates[pivote],tweets[izq],tweets[pivote],hashtags[izq],hashtags[pivote]); 
    limit = izq; //valor inicial de limit 
    
    for(int i=izq+1; i<der; i++){
        if(strcmp(tweets[i],tweets[izq])<0){ //alfabeticamente tweets
            limit++; //ahora esta 1 mas a la derecha 
    ...
    quick_sort(dates, tweets, hashtags, izq, limit -1); //quiero q ordene la izq y la derecha ,, el 4 ya estaria en su posicion ya no se ordena (-1) 
...
void swap2(int &date1, int &date2,char *&tweet1, char*&tweet2, char**&hashtags1, char**&hashtags2){
    int aux_date;   //necesitamos auxiliares
    char * aux_tweet;
    char ** aux_hashtags;
...
//PARA ORDENAR HASHTAGS -> solo le pasaria eso 